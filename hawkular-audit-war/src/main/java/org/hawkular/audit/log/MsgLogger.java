...
@ValidIdRange(min = 99000, max = 99999)
public interface MsgLogger {
    @LogMessage(level = Level.INFO)
    @Message(id = 99000, value = "Audit subsystem starting up")
    void infoAuditSubsystemStartingUpNot();

    @LogMessage(level = Level.ERROR)
    @Message(id = 99001, value = "Cannot send initial audit subsystem startup message: %s")
